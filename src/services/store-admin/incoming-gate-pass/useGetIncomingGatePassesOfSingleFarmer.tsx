...
import type {
  GetIncomingGatePassesByFarmerData,
...
type GetIncomingGatePassesError = {
  success?: boolean;
  message?: string;
...
function getFetchErrorMessage(
  data: GetIncomingGatePassesError | undefined
): string {
  return (
    data?.error?.message ??
    data?.message ??
...
async function fetchIncomingGatePassesByFarmer(
  ...
  try {
    const { data } = await storeAdminAxiosClient.get<
      | { success: true; data: GetIncomingGatePassesByFarmerData }
      | GetIncomingGatePassesError
    ...
    const responseData =
      ...
      typeof err === 'object' &&
      'response' in err &&
      (err as { response?: { data?: GetIncomingGatePassesError } }).response
        ?.data;
...
/** Prefetch incoming gate passes for a farmer – e.g. before opening a form */
