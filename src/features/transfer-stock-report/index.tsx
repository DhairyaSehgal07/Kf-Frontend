...
const TransferStockReportPage = () => {
  ...
  const [appliedParams, setAppliedParams] =
    useState<TransferStockReportParams>({})
  ...
  const [columnVisibility, setColumnVisibility] =
    useState<ColumnVisibilityState>(
      () =>
        getStoredTransferStockReportColumnState(TRANSFER_STOCK_REPORT_COLUMN_IDS)
          .columnVisibility,
    )
  ...
  const table = useTable<ReportFeatures, TransferStockReportRow>({
    ...
    getRowId: (row) =>
      row._id || `${row.gatePassNo}-${row.date}-${row.manualGatePassNumber}`,
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-border bg-card px-4 py-3 shadow-sm">
          ...
          <p className="tabular-nums text-xl font-semibold text-foreground">
            {isLoading
              ? "Loading..."
              : formatIndianIntegerTotal(reportTotals.uniqueFromAccounts)}
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card px-4 py-3 shadow-sm">
          ...
          <p className="tabular-nums text-xl font-semibold text-foreground">
            {isLoading
              ? "Loading..."
              : formatIndianIntegerTotal(reportTotals.uniqueToAccounts)}
          </p>
        </div>
      </div>
