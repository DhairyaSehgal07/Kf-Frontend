...
const ADVANCED_NUMERIC_COLUMN_IDS = new Set<keyof TransferStockReportRow>([
  "totalBags",
])
...
export const advancedReportGlobalFilterFn: ReportFilterFn<TransferStockReportRow> = (
  ...
  if (gatePassSearch) {
    ...
    const matchesGatePass = normalizeText(row.original.gatePassNo).includes(
      query,
    )
    const matchesManual = normalizeText(
