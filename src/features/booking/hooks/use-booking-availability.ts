...
export function useBookingAvailability() {
  ...
  const cachedBooked = queryClient.getQueryData<SummaryVariety[]>(bookingKeys.summary());
  ...
  const availabilityMap = useMemo(
    () => buildNetAvailabilityMap(storageQuery.data ?? [], bookedQuery.data ?? []),
  ...
  const isReady =
    !isLoading && (availabilityMap.size > 0 || (storageQuery.isFetched && bookedQuery.isFetched));
