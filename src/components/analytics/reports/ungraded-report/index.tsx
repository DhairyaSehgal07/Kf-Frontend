...
import {
  ...
  incomingGatePassesQueryOptions,
...
/** Only show gate passes that are not graded (gradingSummary.graded !== true). */
function isUngraded(
  pass: IncomingGatePassWithLink & { gradingSummary?: { graded?: boolean } }
): boolean {
...
function mapGatePassesToRows(gatePasses: IncomingPass[]): IncomingReportRow[] {
  return gatePasses.map((pass) => {
    const bagsFromSizes = (pass.bagSizes ?? []).reduce<number>(
...
const UngradedReportTable = () => {
  ...
  const { data, isLoading, error } = useGetIncomingGatePasses({
    limit: 5000,
  ...
  const rows = useMemo((): IncomingReportRow[] => {
    if (!data?.list) return [];
    const ungradedList = (data.list as IncomingPass[]).filter(isUngraded);
    return mapGatePassesToRows(ungradedList);
  }, [data]);
    ...
    try {
      ...
      const blob = await pdf(
        <IncomingReportTablePdf
          ...
          reportTitle="Ungraded Gate Pass Report"
          rows={rows}
