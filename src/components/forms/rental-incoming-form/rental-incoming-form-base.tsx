...
import { RentalSummarySheet } from './rental-summary-sheet';
...
/** Payload built from the form and passed to onSubmit (create). */
export type RentalIncomingFormSubmitPayload = {
...
export interface RentalIncomingFormBaseProps {
  mode: 'create';
  onSubmit: (payload: RentalIncomingFormSubmitPayload) => void | Promise<void>;
...
const quantitySizes = [...GRADING_SIZES];

export const RentalIncomingFormBase = memo(function RentalIncomingFormBase({
  onSubmit: onSubmitProp,
  voucherNumberDisplay,
  isSubmitting = false,
  gatePassNoForSummary = 0,
  isLoadingVoucher = false,
}: RentalIncomingFormBaseProps) {
  const {
    data: farmerLinks,
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const dateIso = formatDateToISO(value.date).slice(0, 10);

      const payload: RentalIncomingFormSubmitPayload = {
  ...
  const quantityRowsForSummary = useMemo(() => {
    ...
    <main className="font-custom mx-auto max-w-2xl px-4 py-6 sm:px-8 sm:py-12">
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4">
          ...
          >
            {step === 1 ? 'Next' : isSubmitting ? 'Submitting…' : 'Review'}
          </Button>
        </div>
      </form>
